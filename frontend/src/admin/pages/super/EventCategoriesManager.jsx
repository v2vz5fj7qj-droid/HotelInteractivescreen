...
const EMPTY = { key_name: '', label_fr: '', label_en: '', icon: '🗓️', color: '#6B7280', display_order: 0, hotel_id: '' };
...
export default function EventCategoriesManager() {
  ...
  const load = useCallback(async () => {
    try {
      const [{ data: catData }, { data: hotelData }] = await Promise.all([
        api.get('/super/event-categories'),
  ...
  const save = async () => {
    ...
    try {
      ...
      if (modal === 'create') {
        await api.post('/super/event-categories', payload);
        showToast('Catégorie créée');
      } else {
        await api.put(`/super/event-categories/${modal.id}`, payload);
  ...
  const handleDelete = async () => {
    try {
      await api.delete(`/super/event-categories/${confirm.id}`);
  ...
  return (
    <div>
      <div className={styles.managerHeader}>
        <div>
          <h1 className={styles.managerTitle}>Catégories — Agenda</h1>
          <p className={styles.managerSub}>Catégories globales ou propres à un hôtel pour les événements</p>
        </div>
          ...
          <div className={styles.modal} onClick={e => e.stopPropagation()}>
            <div className={styles.modalHeader}>
              <span className={styles.modalTitle}>
                {modal === 'create' ? 'Nouvelle catégorie — Agenda' : `Modifier — ${modal.label_fr}`}
              </span>
      ...
      <ConfirmModal
        ...
        message="Cette catégorie sera définitivement supprimée. Les événements utilisant cette catégorie devront être réassignés."
        danger
