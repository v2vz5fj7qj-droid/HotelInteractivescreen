...
const EMPTY = { key_name: '', label_fr: '', label_en: '', icon: '📋', color: '#6B7280', display_order: 0, hotel_id: '' };

export default function InfoCategoriesManager() {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (modal === 'create') {
        await api.post('/super/info-categories', payload);
        showToast('Catégorie créée');
      } else {
        await api.put(`/super/info-categories/${modal.id}`, payload);
  ...
  const handleDelete = async () => {
    try {
      await api.delete(`/super/info-categories/${confirm.id}`);
  ...
  return (
    <div>
      <div className={styles.managerHeader}>
        <div>
          <h1 className={styles.managerTitle}>Catégories — Infos utiles</h1>
          <p className={styles.managerSub}>Catégories globales ou propres à un hôtel pour les contacts et informations pratiques</p>
        </div>
          ...
          <div className={styles.modal} onClick={e => e.stopPropagation()}>
            <div className={styles.modalHeader}>
              <span className={styles.modalTitle}>
                {modal === 'create' ? 'Nouvelle catégorie — Infos utiles' : `Modifier — ${modal.label_fr}`}
              </span>
      ...
      <ConfirmModal
        ...
        message="Cette catégorie sera définitivement supprimée. Les contacts utilisant cette catégorie devront être réassignés."
        danger
